...
const starCount = 80;
const starSpeed = range(100, 500);
const starBoxSize = 2000;
const excludeRadius = 0.0; 
const starBrightnessRange = range(0.1, 0.8)
const starSizeRange = range(5.0, 25.0);
const cameraFadeDistance = 100;
...
// // yellow
// const colorScaleRangeRG = float(0.9).add(range(float(0.0), float(0.1)));
// const colorScaleRangeB = range(float(0.6), float(0.9));
// const starColorRange = vec3(
//   colorScaleRangeRG,
//   colorScaleRangeRG,
//   colorScaleRangeB
...
// // Create a single random factor per star instance (0.0 to 1.0)
// const starType = range(float(0.0), float(1.0));

// // Mix from a hot blue star to a bright white/yellow star based on that factor
// const blueStar = vec3(0.3, 0.6, 1.0);
// const whiteYellowStar = vec3(1.0, 0.9, 0.7);

// const starColorRange = blueStar.mix(whiteYellowStar, starType);

const tealR = range(float(0.0), float(0.1));                  // Keep red near zero
const tealG = float(0.9).add(range(float(0.0), float(0.1)));  // Max green for neon punch
const tealB = float(0.85).add(range(float(0.0), float(0.15))); // High blue to shift green to teal
const starColorRange = vec3(tealR, tealG, tealB);
